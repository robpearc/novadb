"""AlphaFold3 constants from Supplementary Tables.

Contains all chemical component codes from the AF3 supplement:
- Table 9: Crystallization aids
- Table 10: Ligand exclusion list  
- Table 11: Glycan CCD codes
- Table 12: Ions
- Table 13: Standard residues
- Table 15: PoseBusters common natural ligands

Reference: Abramson et al. 2024, Nature. Supplementary Information.
"""
...
# =============================================================================
# Table 9: Crystallization Aids
# Small molecules commonly added during crystallization (filtered during training)
# =============================================================================
...
# =============================================================================
# Table 10: Ligand Exclusion List
# Common buffer components, salts, and artifacts to exclude from training
# =============================================================================

LIGAND_EXCLUSION_LIST: Final[FrozenSet[str]] = frozenset({
...
# Carbohydrate residues for glycan detection
# =============================================================================

GLYCAN_CCD_CODES: Final[FrozenSet[str]] = frozenset({
...
# =============================================================================
# Table 12: Ions
# All ion CCD codes recognized in AF3
# =============================================================================

ION_CCD_CODES: Final[FrozenSet[str]] = frozenset({
...
# =============================================================================
# Table 13: Standard Residues
# Canonical amino acids and nucleotides
# =============================================================================

STANDARD_AMINO_ACIDS: Final[FrozenSet[str]] = frozenset({
    "ALA", "ARG", "ASN", "ASP", "CYS", "GLN", "GLU", "GLY", "HIS", "ILE",
    "LEU", "LYS", "MET", "PHE", "PRO", "SER", "THR", "TRP", "TYR", "VAL",
    "UNK"  # Unknown amino acid
})

STANDARD_RNA_NUCLEOTIDES: Final[FrozenSet[str]] = frozenset({
    "A", "G", "C", "U", "N"  # N = unknown
})

STANDARD_DNA_NUCLEOTIDES: Final[FrozenSet[str]] = frozenset({
    "DA", "DG", "DC", "DT", "DN"  # DN = unknown
})

STANDARD_RESIDUES: Final[FrozenSet[str]] = (
    STANDARD_AMINO_ACIDS | STANDARD_RNA_NUCLEOTIDES | STANDARD_DNA_NUCLEOTIDES
)
...
POSEBUSTERS_COMMON_LIGANDS: Final[FrozenSet[str]] = frozenset({
    ...
    "PRP", "SAH", "SFG", "SIN", "SLB", "TPP", "UD1", "UDP", "UPG", "URI"
})


# =============================================================================
# Training Parameters from AF3 Supplement
# =============================================================================

# Section 2.5: Training cutoff dates
TRAINING_CUTOFF_DATE: Final[str] = "2021-09-30"
TEMPLATE_CUTOFF_DATE: Final[str] = "2021-09-30"

# Section 2.5.4: Filtering parameters
MAX_RESOLUTION: Final[float] = 9.0  # Å
MAX_POLYMER_CHAINS_TRAINING: Final[int] = 300
MAX_POLYMER_CHAINS_EVAL: Final[int] = 1000
MIN_RESOLVED_RESIDUES: Final[int] = 4

# Section 2.5.1: Weighted PDB sampling parameters
ALPHA_PROT: Final[float] = 3.0
ALPHA_NUC: Final[float] = 3.0
ALPHA_LIGAND: Final[float] = 1.0
BETA_CHAIN: Final[float] = 0.5
BETA_INTERFACE: Final[float] = 1.0

# Section 2.5.3: Clustering thresholds
PROTEIN_CLUSTER_IDENTITY: Final[float] = 0.4  # 40%
NUCLEIC_CLUSTER_IDENTITY: Final[float] = 1.0  # 100%
PEPTIDE_CLUSTER_IDENTITY: Final[float] = 1.0  # 100%
PEPTIDE_LENGTH_THRESHOLD: Final[int] = 10  # residues

# Section 2.6: Tokenization
MAX_ATOMS_PER_TOKEN: Final[int] = 24  # Largest standard residue

# Section 2.7: Cropping
INTERFACE_DISTANCE_THRESHOLD: Final[float] = 15.0  # Å

# Table 4: Cropping weights by dataset
CROPPING_WEIGHTS = {
    "weighted_pdb": {
        "contiguous": 0.20,
        "spatial": 0.40,
        "spatial_interface": 0.40,
    },
    "disordered_pdb": {
        "contiguous": 0.20,
        "spatial": 0.40,
...
# Table 3: Dataset sampling weights
DATASET_WEIGHTS = {
    "weighted_pdb": 0.5,
    "disordered_pdb": 0.02,
...
# Section 2.8: Feature dimensions
NUM_TOKEN_TYPES: Final[int] = 32
NUM_MSA_SEQUENCES: Final[int] = 16384
NUM_PAIRED_MSA_SEQUENCES: Final[int] = 8191
NUM_TEMPLATES: Final[int] = 4
MAX_TEMPLATE_HITS: Final[int] = 20

# Training token limits (Table 6)
CROP_SIZE_INITIAL: Final[int] = 384
CROP_SIZE_FINETUNE_1: Final[int] = 640
CROP_SIZE_FINETUNE_2: Final[int] = 768
...
def is_crystallization_aid(ccd_code: str) -> bool:
    """Check if CCD code is a crystallization aid."""
    return ccd_code.upper() in CRYSTALLIZATION_AIDS


def is_excluded_ligand(ccd_code: str) -> bool:
    """Check if CCD code is in the exclusion list."""
    return ccd_code.upper() in LIGAND_EXCLUSION_LIST


def is_glycan(ccd_code: str) -> bool:
    """Check if CCD code is a glycan."""
    return ccd_code.upper() in GLYCAN_CCD_CODES


def is_ion(ccd_code: str) -> bool:
    """Check if CCD code is an ion."""
    return ccd_code.upper() in ION_CCD_CODES


def is_standard_residue(ccd_code: str) -> bool:
    """Check if CCD code is a standard residue."""
    return ccd_code.upper() in STANDARD_RESIDUES


def is_standard_amino_acid(ccd_code: str) -> bool:
    """Check if CCD code is a standard amino acid."""
    return ccd_code.upper() in STANDARD_AMINO_ACIDS


def is_standard_nucleotide(ccd_code: str) -> bool:
    """Check if CCD code is a standard nucleotide."""
    code = ccd_code.upper()
    return code in STANDARD_RNA_NUCLEOTIDES or code in STANDARD_DNA_NUCLEOTIDES


def get_clustering_threshold(residue_type: str, chain_length: int = 100) -> float:
    """Get appropriate clustering threshold for residue type.
    ...
    """
    if is_standard_nucleotide(residue_type):
        return NUCLEIC_CLUSTER_IDENTITY
    elif is_standard_amino_acid(residue_type):
        if chain_length < PEPTIDE_LENGTH_THRESHOLD:
            return PEPTIDE_CLUSTER_IDENTITY
        return PROTEIN_CLUSTER_IDENTITY
    else:
        return 1.0  # Exact match for ligands


def should_filter_for_training(
    ccd_code: str,
    is_crystallography: bool = False,
) -> bool:
    """Check if a component should be filtered for training.
    
    From AF3 Section 2.5.4:
    - Remove crystallization aids if method is crystallography
    - Remove excluded ligands
    """
    code = ccd_code.upper()
    
    if is_excluded_ligand(code):
        return True
    
    if is_crystallography and is_crystallization_aid(code):
        return True
